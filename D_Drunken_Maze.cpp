...
void solve()
{
    ...
    set<tuple<ll,ll,char,ll>>vis;
    queue< pair<ll,pair<pair<ll,ll> , pair<char,ll>>>> q1;   // sx sy pdir cnt
    q1.push({0,{{sx,sy},{'I',0}}});

    while(!q1.empty())
    {
        ll dis=q1.front().first;
        ll x=q1.front().second.first.first;
        ll y=q1.front().second.first.second;
        char pv=q1.front().second.second.first;
        ll pcnt=q1.front().second.second.second;
        q1.pop();
        ...
        for(ll z=0;z<4;z++)
        {
            ...
            if(issafe(nx,ny) && a[nx][ny]==0)
            {
                ll ncnt=0;
                ll ndir=dir[z];
                if(pv==ndir)ncnt=pcnt+1;
                if(vis.find({nx,ny,ndir,ncnt})==vis.end() && ncnt<3)
                {
                    vis.insert({nx,ny,ndir,ncnt});
                    q1.push({dis+1,{{nx,ny},{ndir,ncnt}}});
                }
            }
        }
    }
