...
void update(int node,int i,int j,vector<ll>&tree,ll in,bool b)
{
    if(i==j)
    {
        if(b==1)tree[node]++;
    ...
    if(in<=mid)
    {
        update(2*node+1,i,mid,tree,in,b);
    }
    else
    {
        update(2*node+2,mid+1,j,tree,in,b);
    }
...
void solve()
{
    ...
    {
        cin>>a[i];
        update(0,0,sz-1,tree,a[i],1);
    }

    ll ans=0;
    for(int i=0;i<n;i++)
    {
        update(0,0,sz-1,tree,a[i],0);
        ans+=query(0,0,sz-1,tree,0,a[i]);
    }
    cout<<ans<<endl;
}
