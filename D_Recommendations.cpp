...
bool cmp1(const vector<ll>& a, const vector<ll>& b) {
    if (a[0] == b[0])
        return a[1] > b[1]; // Descending order for second column
    return a[0] < b[0];     // Ascending order for first column
}

bool cmp2(const vector<ll>& a, const vector<ll>& b) {
    if (a[1] == b[1])
        return a[0] < b[0]; // Descending order for second column
    return a[1] > b[1];     // Ascending order for first column
}
...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {   
        ll s=a[i][0],e=a[i][1],in=a[i][2];
        if(i+1<n && s==a[i+1][0] && e==a[i+1][1])
        {
            s1.insert(e);
            continue;
        }
