...
void solve()
{
    ...
    ll l1=abs(a[0]-b[0]);
    ll r1=abs(a[n-1]-b[n-1]);

    ll mini1=INF,mini2=INF;
    for(int i=1;i<n;i++)
    {
        mini1=min(mini1,abs(a[0]-b[i]));
        mini2=min(mini2,abs(b[0]-a[i]));
    }
    
    l1=min(l1,mini1+mini2);
    ll mini3=INF,mini4=INF;
    for(int i=0;i<n-1;i++)
    {
        mini3=min(mini3,abs(a[n-1]-b[i]));
        mini4=min(mini4,abs(b[n-1]-a[i]));
    }
    r1=min(r1,mini3+mini4);
    ll ans=l1+r1;

    //one more case
    ans=min(ans,abs(a[0]-b[n-1])+abs(b[0]-a[n-1]));
    ans=min(ans,abs(a[0]-b[n-1])+mini2+mini3);
    ans=min(ans,abs(b[0]-a[n-1])+mini1+mini4);
    cout<<ans<<endl;
}
