...
ll n;
bool helper(ll in,vll&a,vvl&ls,vll&dp)
{
    if(in>=n)
    {
        if(in==n)return 1;
    ...
    for(auto x:ls[in])
    {
        bool b1=helper(x+1,a,ls,dp);
        if(b1==1)return dp[in]=1;
    }
    bool b2=helper(in+a[in]+1,a,ls,dp);
    if(b2==1)return dp[in]=1;
    return dp[in]=0;
}
...
void solve()
{
    ...
    {
        ll pv=i-a[i];
        if(pv>=0)ls[pv].push_back(i);
    }
    vll dp(n,-1);
    bool ans=helper(0,a,ls,dp);
