...
{
    ll n;cin>>n;
    vector<pair<pair<char,ll>,pair<char,ll>>>a;
    for(int i=0;i<n;i++)
    {
        char c1,c2;
        ll u,v;
        cin>>c1>>u>>c2>>v;
        a.push_back({{c1,u},{c2,v}});
    } 

    vvl dp(n+1,vll(2,1));  
    for(int i=n-1;i>=0;i--)
    {
        ll opl=a[i].first.first;
        ll opr=a[i].second.first;
        ll val1=a[i].first.second;
        ll val2=a[i].second.second;
        dp[i][0]=dp[i+1][0];
        dp[i][1]=dp[i+1][1];
        if(opl=='x')dp[i][0]+=(val1-1)*max(dp[i+1][0],dp[i+1][1]);
        if(opr=='x')dp[i][1]+=(val2-1)*max(dp[i+1][0],dp[i+1][1]);
    }
