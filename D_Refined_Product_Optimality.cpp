...
void solve()
{
    ...
    while(q--)
    {
        ...
        {
            ll pval=a[x];
            ll ppos=upb(v1,pval)-1;
            pmini=min(v1[ppos],v2[ppos]);
            a[x]++;
            v1[ppos]++;
            nmini=min(v1[ppos],v2[ppos]);
        }
        else
        {
            ll pval=b[x];
            ll ppos=upb(v2,pval)-1;
            pmini=min(v1[ppos],v2[ppos]);
            b[x]++;
            v2[ppos]++;
            nmini=min(v1[ppos],v2[ppos]);
        ...
        if(pmini!=nmini)
        {
            res= ((res%MOD)*modinv(pmini,MOD))%MOD; 
            res=((res%MOD)*(nmini%MOD))%MOD;         
        }
        cout<<res<<" ";
    }
    cout<<endl;
