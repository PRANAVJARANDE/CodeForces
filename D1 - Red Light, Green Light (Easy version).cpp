...
void solve()
{
    ...
    queue<pair<ll,pair<ll,ll>>>q1;
    for(int i=0;i<k;i++)
    {
        if(i==d[0])q1.push({0,{i,1}});
        else q1.push({0,{i,-1}});
        if(i==d[n-1])q1.push({n-1,{i,-1}});
        else q1.push({n-1,{i,1}});
    }
    vector<vvl>vis(n,vvl(k,vll(2,0)));
    while(!q1.empty())
    {
        ll in=q1.front().first;
        ll ctime=q1.front().second.first;
        ll dir=q1.front().second.second;
        q1.pop();
        if(vis[in][ctime][(dir==-1? 0 :1)])continue;
        vis[in][ctime][(dir==-1? 0 :1)]=1;
    
        if(dir==-1)
        {
            if(in+1>=n)continue;
            ll reqt=abs(p[in+1]-p[in]);
            ctime=(((ctime-reqt)%k)+k)%k;
            if(ctime==d[in+1])q1.push({in+1,{ctime,1}});
            else q1.push({in+1,{ctime,-1}});
        }
        else
        {
            if(in-1<0)continue;
    ...
    while(q--)
    {
        ...
        ll ctime=(p[in]-st)%k;
        if(vis[in][ctime][1])cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
}
