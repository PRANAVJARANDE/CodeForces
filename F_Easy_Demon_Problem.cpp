...
void solve()
{
    ...
    set<ll>s1,s2;
    for(auto &x:a)s1.insert(sa-x);
    for(auto &x:b)s2.insert(sb-x);
    ...
    while(q--)
    {
        ...
        for(int i=1;i<=sqrt(x);i++)
        {
            if(x%i==0)
            {
                ...
                if(sn==0)
                {
                    if((s1.count(d1) && s2.count(d2)) || (s1.count(d2) && s2.count(d1)) || (s1.count(-d1) && s2.count(-d2)) || (s1.count(-d2) && s2.count(-d1)))
                    {
                        b=1;
                        cout<<"YES"<<endl;
                    }
