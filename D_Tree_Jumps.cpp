...
void helper(ll in,ll par,ll c,vvl&adj,vvl&lvl,vll&l)
{
    l[in]=c;
    lvl[c].push_back(in);
    for(auto x:adj[in])
...
void solve()
{
    ...
    for(ll i=1;i<n;i++)
    {
        ...
        {
            dp[x]=sum[i-1]%MOD;
            if(i!=1)dp[x]=((dp[x]%MOD)-(dp[fpar[x]]%MOD) + MOD)%MOD;
            sum[i]=(sum[i]%MOD+dp[x]%MOD)%MOD;
        }
    } 
    ll ans=0;
    for(int i=0;i<n;i++)
