...
ll findrootans(ll in,ll par,vvl&adj,map<pair<ll,ll>,ll>&m)
{
    ll cnt=0;
    for(auto x:adj[in])
    {
        ...
        {
            if(m[{in,x}]==0)cnt++;
            cnt+=findrootans(x,in,adj,m);
        }
    }
    return cnt;
}

void helper(ll in,ll par,ll rans,vvl&adj,vll&ans,map<pair<ll,ll>,ll>&m)
{
    ans[in]=rans;
    for(auto x:adj[in])
    {
...
void solve()
{
    ...
    vll res(n,0);
    helper(0,-1,findrootans(0,-1,adj,m),adj,res,m);
    
    ll moves=LLONG_MAX;
    for(int i=0;i<n;i++)moves=min(moves,res[i]);
    
    vll ans;
    for(int i=0;i<n;i++)
    {
        if(res[i]==moves)ans.push_back(i+1);
    }
