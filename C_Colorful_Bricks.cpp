...
ll helper(ll in,ll k,vvl&dp)
{
    ...
    if(in==0)
    {
        return dp[in][k]=((m%MOD) * (helper(in+1,k,dp)%MOD))%MOD;
    }

    //similar to pv
    ll op1=helper(in+1,k,dp)%MOD;

    //diff from pv
    ll op2=0;
    if(k>0)
    {
        op2=(((m-1)%MOD)*(helper(in+1,k-1,dp)%MOD))%MOD;
    }
    return dp[in][k]=((op1%MOD) + (op2)%MOD)%MOD;
}
