...
ll helper(ll i,ll j,ll cj,bool done)
{
    if(i==n-1 && j==m-1 && done)return a[i][(j+cj)%m];
    if(dp[i][j][cj][done]!=-1)return dp[i][j][cj][done];
    if(done)
    {
        ll op1=INF,op2=INF;
        if(j<m-1)op1=a[i][(j+cj)%m]+helper(i,j+1,cj,1);
        if(i<n-1)op2=a[i][(j+cj)%m]+helper(i+1,j,0,0);
        return dp[i][j][cj][done]=min(op1,op2);
    }
    else
    {
        ll ans=INF;
        for(int z=0;z<m;z++)
        {
            ll op=(k*z)+helper(i,j,z,1);
            ans=min(ans,op);
        }
        return dp[i][j][cj][done]=ans;
    }
}
...
void solve()
{
    ...
    dp=vector<vector<vvl>>(n+1,vector<vvl>(m+1,vvl(m+1,vll(2,-1))));
