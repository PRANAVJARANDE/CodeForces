...
ll n,k,z;
ll helper(ll fr,ll bk,vll&a,vvl&dp)
{
    if(fr+bk==k)
    {
        return a[fr-bk];
    }    
    if(dp[fr][bk]!=-1)return dp[fr][bk];
    ll op1=LLONG_MIN;
    ll op2=LLONG_MIN;
    ll in=fr-bk;
    //MOVE FRONT
    if(in+1<n)
    {
        op1=a[in]+helper(fr+1,bk,a,dp);
    }
    //MOVE BACK
    if(in-1>=0 && bk<z)
    {
        op2=a[in]+helper(fr,bk+1,a,dp);
    }
    return dp[fr][bk]=max(op1,op2);
}
...
void solve()
{
    ...
    vvl dp(n+1,vll(z+1,-1));
    ll ans=helper(0,0,a,dp);
    cout<<ans<<endl;
}
