...
ll helper1(ll n,ll x,ll y)
{
    ...
    ll hf=powl(2,n-1); 
    ll num=powl(2,2*n-2);
    //Identify regions   1---2---3---4
    if(x<=hf && y<=hf) return helper1(n-1,x,y);
    if(x>hf && y>hf) return num+helper1(n-1,x-hf,y-hf);
    if(x>hf && y<=hf) return 2*num+helper1(n-1,x-hf,y);
    return 3*num+helper1(n-1,x,y-hf);
}

pair<ll,ll> helper2(ll n,ll d) 
{
    if(n==1)
    {
        if(d==1)return{1,1};
    ...
    if(d<=num) 
    {
        auto pr=helper2(n-1,d);
        x=pr.first;
        y=pr.second;
    }
    else if(d<=2*num) 
    {
        auto pr=helper2(n-1,d-num);
        x=pr.first+hf;
        y=pr.second+hf;
    }
    else if(d<=3*num) 
    {
        auto pr=helper2(n-1,d-2*num);
        x=pr.first+hf;
        y=pr.second;
    }
    else
    {
        auto pr=helper2(n-1,d-3*num);
        x=pr.first;
        y=pr.second+hf;
    }
    return {x,y};
...
void solve()
{
    ...
    while(q--)
    {
        ...
        {
            ll x,y;cin>>x>>y;
            ll ans=helper1(n,x,y);
            cout<<ans<<endl;
        }   
        else
        {
            ll d;cin>>d;
            auto ans=helper2(n,d);
