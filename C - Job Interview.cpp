...
void solve()
{
    ...
    for(int i=0;i<sz;i++)
    {
        if(i<h && a[i]>b[i]) ans.push_back(s-a[i]+a[h]);
        else if(i<h && a[i]<b[i]) ans.push_back(s-b[i]+b[h]);
        else if(i==h) ans.push_back(s);
        else {
            if(fint==1)ans.push_back(s-a[i]+a[h]);
            else ans.push_back(s-b[i]+b[h]);
        }
    }
    for(ll x: ans)
    {
