...
ll ans;
ll dfs(ll in,ll par,vector<vector<ll>>&adj)
{
    ll maxi=0;
    for(ll x: adj[in])
    {
...
void solve()
{
    ...
    while(!q1.empty())
    {
        ll in=q1.front().first.first;
        int mov=q1.front().first.second;
        char col=q1.front().second;
        q1.pop();
