...
bool diee(vector<string>&a,ll i,ll j)
{
    if(a[i][j]!='.')return 1;
    return 0;
}
...
void solve()
{
    ...
    for(int j=0;j<n;j++)
    {
        for(int i=0;i<3;i++)
        {
            if(vis[i][j])
            {
                if(diee(a,i,j+1))continue;
                if(!diee(a,i,j+1) && !diee(a,i,j+2) && !diee(a,i,j+3))vis[i][j+3]=1;
                if(i-1>=0 && !diee(a,i-1,j+1) && !diee(a,i-1,j+2) && !diee(a,i-1,j+3))vis[i-1][j+3]=1;
                if(i+1<3 && !diee(a,i+1,j+1) && !diee(a,i+1,j+2) && !diee(a,i+1,j+3))vis[i+1][j+3]=1;
            }
        }
    }

    for(int j=n-1;j<a[0].size();j++)
    {
        for(int i=0;i<3;i++)
        {
            if(vis[i][j])
