...
bool canAchieveValue(vector<ll>& diff, ll target) {
    map<ll,bool>dp;
    dp[0]=1; 
...
void solve()
{
    ...
    for (int i = 0; i <= n - k; i++) {
        for (int j = 0; j <= m - k; j++) {
            ll gridSum = sum[i + k - 1][j + k - 1];
            if (i > 0) gridSum -= sum[i - 1][j + k - 1];
            if (j > 0) gridSum -= sum[i + k - 1][j - 1];
            if (i > 0 && j > 0) gridSum += sum[i - 1][j - 1];
            ll ta=k*k-2*gridSum;
            if(ta==0)continue;
            gc=__gcd(gc,ta);
        }
