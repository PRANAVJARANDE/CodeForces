...
ll n;
ll maxi;
ll helper(ll in,ll k,vll&a,vvl&dp)
{
    if(k<0)return maxi;
    if(in>=n)return 0;
    if(dp[in][k]!=-1)return dp[in][k];
    ll op1=a[in]+helper(in+1,k,a,dp);
    ll op2=maxi;
    if(in==0)
    {
        for(int i=0;i<min(n-1,k);i++)
        {
            ll mini=a[i+1];
            ll s=(i+1)*mini;
            op2=min(op2,s+helper(i+1,k-(i+1),a,dp));
        }
        for(int i=in+1;i<min(n,k+in+1);i++)
        {
            ll mini=a[in];
            if(i+1<n)mini=min(mini,a[i+1]);
            ll cnt=i-in;
            op2=min(op2,a[in]+(cnt*mini)+helper(i+1,k-cnt,a,dp));
        }
