...
ll helper(ll rem,ll curlf,ll n,char pv)
{   
    if(rem==0)return 0;
    ll hf=(rem-1)/2;
    ll hlf=curlf/2;
    if(pv=='R')
    {
        if(hlf<n)
        {
            ll res=1+hf+helper(hf,hlf,n-hlf,'R');
            return res;
        }
        else
        {
            ll res=1+helper(hf,hlf,n,'L');
            return res;
        }
    }
    else
    {
        if(hlf<n)
        {
            ll res=1+helper(hf,hlf,n-hlf,'R');
            return res;
        }
        else
        {
            ll res=1+hf+helper(hf,hlf,n,'L');
            return res;
        }
    }
}
...
{
    ll h,n;cin>>h>>n;
    ll ans=helper(powl(2,h+1)-1,powl(2,h),n,'R')-1;
    cout<<ans<<endl;
}
