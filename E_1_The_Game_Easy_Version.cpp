...
void calcmax(ll in, ll par, vvl &adj, vll &smax, vll &a) {
    smax[in] = a[in];
    for (auto x : adj[in]) {
        if (x != par) {
            calcmax(x, in, adj, smax, a);
            smax[in] = max(smax[in], smax[x]);
        }
    }
}

ll ans;
void helper(ll in, ll par, ll maxi, vvl &adj, vll &smax, vll &a) {
    if (a[in] < maxi) {
        if (ans == -1 || a[in] > a[ans]) {
            ans = in;
        }
    ...
    for (auto x : adj[in]) {
        if (x != par) {
            ll tp = m1;
            if (smax[x] == m1) tp = m2;
            helper(x, in, max(maxi, max(tp, a[in])), adj, smax, a);
        }
    }
}
...
void solve() {
    ...
    vll smax(n, 0);
    calcmax(0, -1, adj, smax, a);

    ans=-1;
    helper(0, -1, -1, adj, smax, a);
    cout << ans + 1 << endl; 
}
