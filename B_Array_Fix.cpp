...
ll n;
bool helper(int in,ll prev,vector<ll>&a,vector<vector<ll>>&dp)
{
    if(in>=n)return 1;
    //split 
    ...
    if(prev<=a1 && a1<=a2)
    {
        op1=helper(in+1,a2,a,dp);
        if(op1==1)return dp[in][prev]=1;
    }
    //notsplit
    bool op2=0;
    if(prev<=a[in])
    {
        op2=helper(in+1,a[in],a,dp);
        if(op2==1)return dp[in][prev]=1;
    }
    return 0;
}
