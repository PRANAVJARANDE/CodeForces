...
void solve()
{
    ...
    for(int k=0;k<n;k++)
    {
        ...
        for(auto x:v1)
        {
            for(auto y:v1)
            {
                adj[in][x]=min(adj[in][x],adj[in][y]+adj[y][x]);
                adj[x][in]=min(adj[x][in],adj[x][y]+adj[y][in]);
            }
        }
        ll sum=0;
        for(auto x:v1)
        {
            for(auto y:v1)
            {
                adj[x][y]=min(adj[x][y],adj[x][in]+adj[in][y]);
                adj[y][x]=min(adj[y][x],adj[y][in]+adj[in][x]);
                sum+=adj[x][y];
            }
        }
        res[n-k-1]=sum;
