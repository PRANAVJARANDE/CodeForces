...
ll n;
ll helper(ll in,vll&a,vector<string>&s,ll rev,vvl&dp)
{
    if(in>=n)return 0;
    if(dp[in][rev]!=-1)return dp[in][rev];
    string rv=s[in];
    reverse(rv.begin(),rv.end());
    
    string pv=s[in-1];
    if(rev)reverse(pv.begin(),pv.end());
    ll op1=INF,op2=INF;
    if(pv<=s[in])op1=helper(in+1,a,s,0,dp);
    if(pv<=rv)op2=a[in]+helper(in+1,a,s,1,dp);
    return dp[in][rev]=min(op1,op2);
}
...
void solve()
{
    ...
    vvl dp(n,vll(2,-1));
    ll ans=min(helper(1,a,s,0,dp),a[0]+helper(1,a,s,1,dp));
    if(ans==INF)ans=-1;
    cout<<ans<<endl;
}
