...
ll getval(ll i, ll n, ll k) {
    // Compute the value based on the formula provided
    ll term1 = (i + 1) * (2 * k + i) / 2;
    ll term2 = (n - i - 1) * (2 * k + i + n) / 2;
    ll ans = abs(term1 - term2);
    return ans;
}
...
void solve() {
    ...
    while (l <= r) {
        ll mid = l + (r - l) / 2;
        ll v0 = (mid > 0) ? getval(mid - 1, n, k) : INF;
        ll v1 = getval(mid, n, k);
        ll v2 = (mid < n - 1) ? getval(mid + 1, n, k) : INF;
