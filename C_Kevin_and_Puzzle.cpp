...
ll n;
ll helper(ll in,ll pr,ll cnt,vll&a,vvl&dp)
{
    if(in>=n)return 1;
    if(dp[in][pr]!=-1)return dp[in][pr];
    //pr--0   previos honest
    //pr--1   previous liar
    ll op1=0;
    if(pr==0)
    {
        op1=helper(in+1,1,cnt+1,a,dp)%MOD;
    }
    ll op2=0;
    if(a[in]==cnt)
    {
        op2=helper(in+1,0,cnt,a,dp)%MOD;
    }
    return dp[in][pr]=((op1%MOD)+(op2%MOD))%MOD;
}
