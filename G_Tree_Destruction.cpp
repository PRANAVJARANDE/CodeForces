...
vvl dp;
void helper(ll in,ll par,vvl &adj,vll & deg)
{
    ll maxi1=-1,maxi2=-1;
    for(auto x:adj[in])
    {
        ...
        {
            helper(x,in,adj,deg);
            ll ta=dp[x][0]+deg[in]-2;
            if(dp[x][0]>maxi1)
            {
                maxi2=maxi1;
                maxi1=dp[x][0];
            }
            else if(dp[x][0]>maxi2)
            {
                maxi2=dp[x][0];
            }
            dp[in][0]=max(dp[in][0],ta);
            dp[in][1]=max(dp[in][1],ta);
        }
    }
    if(maxi1!=-1 && maxi2!=-1)
    {
        dp[in][1]=maxi1+maxi2+deg[in]-4;
    }
}
