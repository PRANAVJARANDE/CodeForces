...
void solve()
{
    ...
    for(int i=0;i<n-1;i++)
    {
        if(a[i]==a[i+1])
        {
            if(i-1>=0 && ps[i-1][0]<2*a[i])
            {
                cout<<a[i]<<" "<<a[i+1]<<" "<<a[ps[i-1][1]]<<" "<<a[ps[i-1][1]-1]<<endl;
                return;
            }
            if(i+2<n && ns[i+2][0]<2*a[i])
            {
                cout<<a[i]<<" "<<a[i+1]<<" "<<a[ns[i+2][1]]<<" "<<a[ns[i+2][1]+1]<<endl;
                return;
            }
            if(i-1>=0 && i+2<n && a[i+2]-a[i-1]<2*a[i])
            {
                cout<<a[i]<<" "<<a[i+1]<<" "<<a[0]<<" "<<a[i+2]<<endl;
                return;
            }
        }
    }
