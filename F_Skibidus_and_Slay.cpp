...
void helper(ll in,ll par,vvl&adj,string&ans,vll&a)
{
    map<ll,ll>m;
    for(auto x:adj[in])
    {
        ...
        {
            m[a[x]]++;
            if(m[a[x]]==2)ans[a[x]]='1';
            if(a[in]==a[x])ans[a[in]]='1';
            if(par!=-1)
            {
                if(a[par]==a[x])ans[a[x]]='1';
            }
            helper(x,in,adj,ans,a);
        }
    }
}
