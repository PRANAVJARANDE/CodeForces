...
void solve()
{
    ...
    vvl maxi(n,vll(m,LLONG_MIN));
    vvl mini(n,vll(m,LLONG_MAX));
    maxi[0][0]=a[0][0];
    mini[0][0]=a[0][0];
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            if(i-1>=0)
            {
                maxi[i][j]=max(maxi[i][j],maxi[i-1][j]+a[i][j]);
                mini[i][j]=min(mini[i][j],mini[i-1][j]+a[i][j]);
            }
            if(j-1>=0)
            {
                maxi[i][j]=max(maxi[i][j],maxi[i][j-1]+a[i][j]);
                mini[i][j]=min(mini[i][j],mini[i][j-1]+a[i][j]);
            }
        }
    }

    if(mini[n-1][m-1]<=0 && 0<=maxi[n-1][m-1])cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
}
