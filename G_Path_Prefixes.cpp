...
void helper(ll in,ll par,ll l,ll av,ll bv,vll&v1,vector<vector<pair<ll,pair<ll,ll>>>>&adj,vll&ans)
{
    for(auto x:adj[in])
    {
        ll ch=x.first;
        ll aval=x.second.first;
        ll bval=x.second.second;
        if(ch!=par)
        {
            ll nav=av+aval;
            ll nbv=bv+bval;
            ...
            v1[l]=nbv;
            ans[ch]=upb(v1,nav);
            helper(ch,in,l+1,nav,nbv,v1,adj,ans);
            v1[l]=temp;
        }
    }
}
