...
void solve()
{
    ...
    map<ll, map<ll, vector<vll>>> m;

    while (q--) {
        ll a, d, k;
        cin >> a >> d >> k;
        m[d][a % d].push_back({a, a + d * k});
    }
    ...
    for (auto x : m) {
        for (auto y : x.second) {
            ...
            vector<vll> rng;
            rng.push_back(y.second[0]);

            for (int i = 1; i < y.second.size(); i++) {
                if (y.second[i][0] <= rng.back()[1]) {
                    rng.back()[1] = max(rng.back()[1], y.second[i][1]);
                } else {
                    rng.push_back(y.second[i]);
                }
            }

            for (auto r : rng) {
                for (ll i = r[0]; i <= min(r[1], n); i += x.first) {
                    d1.union_set(r[0], i);
                }
            }
        }
