...
pair<ll,ll> helper(ll in,ll par,vll&deg,vvl&adj)
{
    ll ans=0;
    ll m1=0,m2=0;
    for(auto x:adj[in])
    {
        if(x!=par)
        {
            ans=max(ans,deg[x]+deg[in]-2);
            auto pr=helper(x,in,deg,adj);
            ll cmax=pr.first;
            ll cans=pr.second;
            ans=max(ans,cmax+deg[in]-1);
            cmax=max(cmax,deg[x]);
            if(m1<cmax)
            {
                m2=m1;
