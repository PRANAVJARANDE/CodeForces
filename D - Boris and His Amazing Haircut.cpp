...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        val[b[i]].push_back(i);
        if(a[i]!=b[i])to_change[b[i]].push_back(i);
    }

    vll v1;
    for(auto x:val)v1.push_back(x.first);
    sort(v1);
    ...
    for(auto num:v1)
    {
        if(!to_change[num].empty())
        {
            ll cnt=1;
            ll pv=to_change[num][0];
            for(int i=1;i<to_change[num].size();i++)
            {
                ll sum=query(0,0,n-1,tree,pv,to_change[num][i]);
                if(sum!=0)
                {
                    cnt++;
                    pv=to_change[num][i];
        ...
        for(auto x:val[num])update(0,0,n-1,tree,x);
    }
    cout<<"YES"<<endl;
}
