...
ll n,m,q;
int helper(ll pos,map<ll,set<ll>> f1,vll&a)
{
    ll diff=0;
    ll nxin=pos+1;
    ll prev=pos-1;
    if(nxin<n && nxin>=0)
    {
        ll beg=*f1[a[pos]].begin();
        ll beg1=*f1[a[nxin]].begin();
        if(beg<=beg1)diff++;
    }
    if(prev<n && prev>=0)
    {
        ll beg=*f1[a[prev]].begin();
        ll beg1=*f1[a[pos]].begin();
        if(beg<=beg1)diff++;
    }
    return diff;
}
...
void solve()
{
    ...
    {
        ll beg=*f1[a[i]].begin();
        ll beg1=*f1[a[i+1]].begin();
        if(beg<=beg1)ans++;
    }
    if(ans==n-1)cout<<"YA"<<endl;
    ...
    while(q--)
    {
        ...
        if(ini!=s)
        {
            ans-=helper(pos[ini],f1,a);
            f1[ini].erase(t);
            ans+=helper(pos[ini],f1,a);
            ans-=helper(pos[s],f1,a);
            f1[s].insert(t);
            ans+=helper(pos[s],f1,a);
            b[t]=s;
        }
