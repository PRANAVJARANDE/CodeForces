...
ll findJustGreater(vector<ll>& arr, int k) {
...
ll findJustSmaller(vector<ll>& arr, ll k) {
    int low = 0, high = arr.size() - 1;
    int ans = -1;
    ...
    while(q--)
    {
        ...
        for(int i=0;i<2;i++)
        {
            for(int j=0;j<2;j++)
            {
                ...
                op += s2[j];
                sort(op);
                ll p1=findJustGreater(m[op],u);
                if(p1!=-1)
                {
                    ll res=abs(p1-u)+abs(p1-v);
                    ans=min(ans,res);
                }
                ll p2=findJustSmaller(m[op],u);
                if(p2!=-1)
                {
                    ll res=abs(p2-u)+abs(p2-v);
                    ans=min(ans,res);
                }
                ll p3=findJustGreater(m[op],v);
                if(p3!=-1)
                {
                    ll res=abs(p3-u)+abs(p3-v);
                    ans=min(ans,res);
                }
                ll p4=findJustSmaller(m[op],v);
                if(p4!=-1)
                {
                    ll res=abs(p4-u)+abs(p4-v);
                    ans=min(ans,res);
                }
