...
ll dfs(ll k,ll in,ll par,vvl&adj,vvl&a)
{
    ll req=max(0,k-a[0][1]);
    for(auto x:adj[in])
    {
        if(x!=par)
        {
            ll diff=req-max(0,k-a[x][1]);
            ll st=a[x][1]-diff;
            if(st<a[x][0])
            {
                return 0;
            }
            bool b=dfs(k,x,in,adj,a);
            if(b==0)return 0;
        }
    }
    return 1;
...
ll ans;
ll dfs(ll in,ll par,vvl&adj,vvl&a)
{
    vll v1;
    ll maxi=a[in][0];
    for(auto x:adj[in])
    {
        if(x!=par)
        {
            ll nxval=dfs(x,in,adj,a);
            maxi=max(maxi,nxval);
            v1.push_back(nxval);
        }
    }
    maxi=min(maxi,a[in][1]);
    for(auto x:v1)ans+=max(0,x-maxi);
    return maxi;
}
